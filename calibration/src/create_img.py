...
def subscriberCallBack(msg):
    ...
    for i in range(np.shape(xyz_array)[0]):
        if math.isnan(xyz_array[i][2]) or math.isinf(xyz_array[i][2]):
            print(xyz_array[0])
...
def listener():
    ...
    rospy.Subscriber("/cmu_rc3/velodyne_cloud_registered", pc2, subscriberCallBack) #change topic name as required
    rospy.spin()
